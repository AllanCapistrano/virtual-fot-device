...
public class Main {
  ...
  public static void main(String[] args) {
    try (
      InputStream input = Main.class.getResourceAsStream("broker.properties")
    ) {
      if (input == null) {
      ...
      String port = CLI.getPort(args).orElse(props.getProperty("port"));

      String password = CLI
        .getPassword(args)
        .orElse(props.getProperty("password"));

      String user = CLI.getUsername(args).orElse(props.getProperty("username"));

      String timeout = CLI.getTimeout(args).orElse("10000");

      String deviceConduct = CLI.getDeviceConduct(args).orElse("1");

      if (
        Integer.parseInt(deviceConduct) < 1 ||
        Integer.parseInt(deviceConduct) > 5
      ) {
        deviceConduct = "1";
        System.err.println(
          "The deviceConduct is out of range [1 -> Honest, 2 -> Malicious, 3 -> Selfish, 4 -> Corrupt, 5 -> Disturbing]. So it's using the default value (1)."
        );
      }
      ...
      FoTDevice device = new FoTDevice(
        deviceId,
        Integer.parseInt(deviceConduct),
        sensors
      );
  ...
  private static List<FoTSensor> readSensors(
    String fileName,
    String deviceName
  ) throws IOException {
    try (
      var inputStream = Main.class.getResourceAsStream(fileName);
